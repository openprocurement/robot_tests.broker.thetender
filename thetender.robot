...
Підготувати клієнт для користувача
    ...
    Run Keyword If    '${username}' != 'thetender_Viewer'    Login    ${username}
...
Створити об'єкт МП
    ...
    Run keyword    thetender.Змінити дані профілю органу приватизації    ${tender_data.data.assetCustodian}
    ...
    Run keyword    thetender.Оновити сторінку з об'єктом МП    ${username}    ${assetID}
    [Return]    ${assetID}
...
Оновити сторінку з об'єктом МП
    ...
    Run keyword    thetender.Оновити дані
    Sleep    1
...
Пошук лоту по ідентифікатору
    [Arguments]    ${username}    ${tender_uaid}
    Run keyword    thetender.Оновити сторінку з лотом    ${username}    ${tender_uaid}

Пошук об’єкта МП по ідентифікатору
    [Arguments]    ${username}    ${tender_uaid}
    Run keyword    thetender.Оновити сторінку з об'єктом МП    ${username}    ${tender_uaid}
...
Отримати інформацію з активу в договорі
    ...
    ${return_value}=    Run Keyword    thetender.Отримати інформацію з активу ${item_id} контракту про ${field_name}
    [Return]    ${return_value}
...
Отримати інформацію із об'єкта МП
    ...
    ${return_value}=    Run Keyword    thetender.Отримати інформацію про ${field_name}
    [Return]    ${return_value}
...
Завантажити ілюстрацію в об'єкт МП
    ...
    Run keyword    thetender.Пошук об’єкта МП по ідентифікатору    ${username}    ${tender_uaid}
...
Завантажити ілюстрацію в лот
    [Arguments]    ${username}    ${tender_uaid}    ${filepath}
    Run keyword    thetender.Пошук лоту по ідентифікатору    ${username}    ${tender_uaid}
...
Завантажити документ в умови проведення аукціону
    ...
    Run keyword    thetender.Завантажити документ в лот з типом    ${username}    ${tender_uaid}    ${filepath}    ${documentType}

Внести зміни в об'єкт МП
    [Arguments]    ${username}    ${tender_uaid}    ${field_name}    ${field_value}
    Run keyword    thetender.Змінити ${field_name} об'єкта МП    ${username}    ${tender_uaid}    ${field_value}

Внести зміни в лот
    [Arguments]    ${username}    ${tender_uaid}    ${fieldname}    ${fieldvalue}
    Run keyword    thetender.Змінити ${fieldname} лоту    ${username}    ${tender_uaid}    ${field_value}

Внести зміни в актив лоту
    [Arguments]    ${username}    ${item_id}    ${tender_uaid}    ${fieldname}    ${fieldvalue}
    Run keyword    thetender.Змінити поле ${fieldname} активу лоту    ${field_value}

Внести зміни в умови проведення аукціону
    [Arguments]    ${username}    ${tender_uaid}    ${fieldname}    ${fieldvalue}    ${auction_index}
    Run keyword    thetender.Змінити поле ${fieldname} аукціону ${auction_index}    ${field_value}
...
Внести зміни в актив об'єкта МП
    ...
    Run keyword    thetender.Змінити ${field_name} об'єкта МП    ${username}    ${tender_uaid}    ${item_id}    ${field_value}
...
Додати актив до об'єкта МП
    ...
    Run keyword    thetender.Додати предмет    ${item}
...
Додати умови проведення аукціону
  ...
  Run KeyWord  thetender.Додати умови проведення аукціону номер ${index}  ${username}  ${tender_uaid}  ${auction}

Додати умови проведення аукціону номер 0
  [Arguments]  ${username}  ${tender_uaid}  ${auction}
  Log    ${auction}
  thetender.Оновити сторінку з лотом    ${username}  ${tender_uaid}
  Натиснути    id=auction-0-update-btn
...
Отримати інформацію з активу лоту
    [Arguments]    ${username}    ${tender_uaid}    ${item_id}    ${field_name}
    ${return_value}=    Run keyword     thetender.Отримати значення поля ${field_name} щодо активу лоту    ${username}    ${tender_uaid}    ${item_id}
    [Return]    ${return_value}

Отримати інформацію із лоту
    [Arguments]    ${username}    ${tender_uaid}    ${field_name}
    ${return_value}=    Run keyword    thetender.Отримати інформацію щодо лоту про ${field_name}
    [Return]    ${return_value}
...
Пошук тендера по ідентифікатору
    [Arguments]    ${username}    ${tender_uaid}
    Run keyword    thetender.Оновити сторінку з тендером    ${username}    ${tender_uaid}

Отримати інформацію із тендера
    [Arguments]    ${username}    ${tender_uaid}    ${field_name}
    Run keyword    thetender.Оновити сторінку з тендером    ${username}    ${tender_uaid}
    ${return_value}=    Run keyword    thetender.Отримати значення поля ${field_name} тендеру
    [Return]    ${return_value}
...
Отримати інформацію із запитання
    ...
    Run keyword    thetender.Пошук тендера по ідентифікатору    ${username}    ${tender_uaid}
...
Подати цінову пропозицію
    ...
    Run keyword    thetender.Змінити дані профілю учасника    ${bid.data.tenderers[0]}
    Run keyword    thetender.Пошук тендера по ідентифікатору    ${username}    ${tender_uaid}
...
Скасувати закупівлю
    ...
    Input text    id=cancellations-reason    ${cancellation_reason}
...
Отримати інформацію із документа
    ...
    ${return_value}=    Run keyword    thetender.Отримати значення поля ${field} документу    ${doc_id}
    [Return]    ${return_value}
...
Отримати інформацію із пропозиції
    ...
    Run keyword    thetender.Пошук тендера по ідентифікатору    ${username}    ${tender_uaid}
    Натиснути    id=bid-create-btn
    ${return_value}=    Run keyword    thetender.Отримати інформацію з пропозиції шодо ${field}
    [Return]    ${return_value}
...
Активувати контракт
    ...
    Run keyword    thetender.Пошук тендера по ідентифікатору    ${username}    ${contract_uaid}
